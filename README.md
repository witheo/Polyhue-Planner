...
Backlog includes **Import from text…**, which calls a small **local Node proxy** (the browser cannot run the Cursor CLI). Spike notes and wire format: [`docs/brain-dump-provider-spike.md`](./docs/brain-dump-provider-spike.md).

Quick test with the built-in **mock** proxy (one task per non-empty line, 30 minutes each):
...
| `npm run brain-dump-proxy` | Mock `POST /brain-dump` on port 8787 (dev only) |
